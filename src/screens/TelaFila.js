      ...
      render(){
          if (this.state.fila.length == 1) {
                ...
                // anima as pessoas
                    ...
                    setTimeout(
                        () => {
                            let x = [...this.state.fila];
                            //os 3 pontos quer dizer que ele vai criar um vetor novo, clonar pra nao dar erro de render
                            x.pop();
                            //setFila(getFilaByCPF(cpf)) 
                            this.setState({fila:x});
                        }, 1000
                    );
                }
        let marq = 0
    
        let usuarios = this.state.fila.map(
            (elem, key) => {
                if (key == 0) {
                    return <View key={key} style={styles.containerPessoa}><FilaInfo cor="black" /><Text>{this.state.fila.length - key}</Text></View>
                }
                if (elem.ehApp) {
                    return <View key={key} style={styles.containerPessoa}><FilaInfo cor="blue" /><Text>{this.state.fila.length - key}</Text></View>
                } else {
                    return <View key={key} style={styles.containerPessoa}><FilaInfo cor="red" /><Text>{this.state.fila.length - key}</Text></View>
                }
            }
        )
